...
- (void)setTitle:(NSString *)title atIndex:(NSInteger)index{
    ...
    [temp addObjectsFromArray:_sectionTitles];
    [temp replaceObjectAtIndex:index withObject:title];
    self.sectionTitles = [temp mutableCopy];
}
...
- (CGSize)measureTitleAtIndex:(NSUInteger)index {
    ...
    if ( _hasMax && size.width > _maxWidth) {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            rect = CGRectMake(ceilf(rect.origin.x), ceilf(rect.origin.y)+2, ceilf(rect.size.width), ceilf(rect.size.height));
    ...
    if (self.selectedSegmentIndex != HMSegmentedControlNoSegment) {
        if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
            ...
            if (!self.selectionIndicatorStripLayer.superlayer) {
                CGRect f =[self frameForSelectionIndicator];
                f.size.width += 20;
                f.origin.x -= 9;
                self.selectionIndicatorStripLayer.frame = f;
                self.selectionIndicatorStripLayer.masksToBounds = YES;
                self.selectionIndicatorStripLayer.cornerRadius = f.size.height/2;
...
- (NSDictionary *)resultingTitleTextAttributes {
    NSDictionary *defaults = @{
        NSFontAttributeName : [UIFont fontWithName:@"FZLanTingHeiS-R-GB" size:15.0f],
